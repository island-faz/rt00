...
** Last update Wed Jun  4 10:51:49 2014 
*/
...
t_dpoint	*inside_triangle(t_dpoint *intr, t_triangle *tr, t_dpoint *pos)
{
  char		b1;
  char		b2;
  char		b3;
...
t_dpoint	*triangle_ray_inter(t_triangle *tr, t_dpoint *pos, t_ray *ray)
{
  ...
  plane.normal = tr->v1->normal;
  plane.coor = new_dpoint(tr->v1->point->x + pos->x,
			  tr->v1->point->y + pos->y,
			  tr->v1->point->z + pos->z);
  ...
  if (!inters)
    return (0x0);
  return(inside_triangle(inters, tr, pos));
}
